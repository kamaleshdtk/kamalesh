...
const ReportCard: React.FC<{report: AnalysisReport; onView: (report: AnalysisReport) => void; status?: 'Approved' | 'Needs Fix'}> = ({ report, onView, status }) => {
    ...
    const statusStyles = {
        ...
        'Needs Fix': 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300'
    };

    return (
        <button
          className="group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-primary rounded-lg shadow-md transition-all duration-300 cursor-pointer overflow-hidden text-left w-full flex flex-col"
        ...
        >
            ...
            <div className="p-4 flex flex-col flex-grow">
                <div className="flex-grow">
                    <p className="font-bold text-text-primary dark:text-white truncate" title={report.input_value}>
                      {displayName}
                    </p>
                    <p className="text-sm text-text-secondary dark:text-gray-400 mt-1">{formatDate(report.created_at)}</p>
                ...
                <div className="flex justify-end items-baseline pt-4">
                    ...
                    <span className="text-xl font-semibold text-text-secondary dark:text-gray-500">/100</span>
                </div>
            </div>
        </button>
    );
