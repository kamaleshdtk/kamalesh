...
const ScoreCard: React.FC<{ score: number; label: string; summary: string }> = ({ score, label, summary }) => {
    ...
    return (
        <div className={`p-6 rounded-xl shadow-soft flex-1 ${getBgColor(score)}`}>
            <div className="flex flex-col text-center sm:text-left sm:flex-row items-center gap-4">
                <div className="relative w-24 h-24 flex-shrink-0">
                    <svg className="transform -rotate-90" width="100%" height="100%" viewBox="0 0 100 100">
                        ...
                        <circle className={getRingColor(score)} strokeWidth="10" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" stroke="currentColor" fill="transparent" r="45" cx="50" cy="50" />
                    </svg>
                    <span className="absolute inset-0 flex items-center justify-center text-2xl font-bold text-text-primary">{score}</span>
                </div>
                <div>
...
const ReportPage: React.FC<{ report: AnalysisReport; onBack: () => void }> = ({ report, onBack }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      <button onClick={onBack} className="flex items-center gap-2 text-sm text-text-secondary hover:text-primary mb-6 font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-1">
            <div className="sticky top-24">
                ...
                <img src={screenshot_url} alt="Analyzed Screenshot" className="rounded-xl shadow-soft w-full" />
                <div className="mt-6 bg-white p-5 rounded-xl shadow-soft">
            ...
            <div className="mb-6">
                <div className="border-b border-gray-200">
                    <nav className="-mb-px flex space-x-4 sm:space-x-8" aria-label="Tabs">
                        {hasUiIssues && <button onClick={() => setActiveTab('UI')} className={`${activeTab === 'UI' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-lg`}>UI Analysis</button>}
                        {hasUxIssues && <button onClick={() => setActiveTab('UX')} className={`${activeTab === 'UX' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-lg`}>UX Analysis</button>}
                    </nav>
                </div>
            </div>
